    ...
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h4 class="fw-bold">دسته بندی تیکت</h4>
            </section>
            ...
            <section
                ...
                <a href="{{ route('ticket.category.create') }}"
                                ...
                                <td>
                                    <label for="">
                                        <input type="checkbox" id="{{ $ticket->id }}"
                                            ...
                                            data-url="{{ route('ticket.category.status', $ticket->id) }}"
                                             @if ($ticket->status === 1) checked @endif>
                                    </label>
                                </td>
                                <td class="width-16 text-left">
                                    <a href="{{ route('ticket.category.edit', $ticket->id) }}"
                                        class="btn btn-primary btn-sm fw-bold">
                                        <i class="fa fa-edit p-1"></i>
                                        ویرایش
                                    </a>
                                    <form class="d-inline"
                                        action="{{ route('ticket.category.destroy', $ticket->id) }}" method="POST">
                                        @csrf
                                        {{ method_field('delete') }}
