                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @foreach ($comments as $comment) 
                                            <tr>   
                                <td>
                                    <p class="truncate-text">{{ $comment->body }}</p>
                                </td>
                                <td>
                                ...
                                <td>
                                    <label for="">
                                        <input type="checkbox" id="{{ $comment->id }}" onchange="ChangeStatus({{ $comment->id }})"
                                            data-url="{{ route('market.comment.status', $comment->id) }}" @if ($comment->status === 1) checked
                                            @endif>
                                    </label>
                                </td>
                                <td class="width-16 text-left">
                                    <a href="{{ route('market.comment.show', $comment->id) }}" class="btn btn-primary btn-sm fw-bold">
                                        <i class="fa fa-eye p-1"></i>
                                        نمایش
                                    </a>
                                    @if ($comment->approved)
                                        <a href="{{ route('market.comment.approved', $comment->id) }}" class="btn btn-warning btn-sm mx-3 fw-bold">
                                            <i class="fa fa-clock p-1"></i>
                                            عدم تایید
                                        </a>
                                    @else  
                                        <a href="{{ route('market.comment.approved', $comment->id) }}" class="btn btn-success btn-sm mx-3 fw-bold">
